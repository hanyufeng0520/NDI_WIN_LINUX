...
template<class _K,
	class _V,
	class _Hasher = std::hash<_K>,
	class _Keyeq = std::equal_to<_K>>
	class SyncUnorderedMap
{
	std::unordered_map<_K, _V, _Hasher, _Keyeq>	m_map;
	SRWLocker									m_lock;

public:
	using iterator = typename std::unordered_map<_K, _V, _Hasher, _Keyeq>::iterator;
	using const_iterator = typename std::unordered_map<_K, _V, _Hasher, _Keyeq>::const_iterator;
	using value_type = typename std::unordered_map<_K, _V, _Hasher, _Keyeq>::value_type;
	using key_type = typename std::unordered_map<_K, _V, _Hasher, _Keyeq>::key_type;
	using size_type = typename std::unordered_map<_K, _V, _Hasher, _Keyeq>::size_type;
	using mapped_type = _V;

	SyncUnorderedMap() = default;
	~SyncUnorderedMap() = default;
	...
	void insert_or_assign(const _K& key, _V& value)
	{
		SRWLockHolder lock(m_lock, false);
